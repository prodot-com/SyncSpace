...
import {
  Plus, X, Briefcase, LogOut, Loader2, Search, Bell,
  LayoutGrid, Users, CheckSquare, Settings, ChevronDown, Clock, Shield,
  Edit3, Lock, FolderKanban, MessageCircle
} from "lucide-react";
import axios from "axios";
import { io } from "socket.io-client";
...
const Modal = ({ children, close, size = 'md' }) => (
    <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-[70] flex justify-center items-center p-4 font-sans animate-fade-in">
        <div className={`bg-slate-800 border border-slate-700 rounded-2xl shadow-2xl shadow-black/50 p-6 sm:p-8 w-full max-w-${size} relative animate-slide-up`}>
            <button onClick={close} className="absolute top-4 right-4 text-slate-400 hover:text-white transition-transform hover:rotate-90 duration-300">
                <X size={24} />
            </button>
            {children}
...
const CreateWorkspaceModal = ({ closeModal, handleCreateWorkspace, isLoading }) => {
    ...
    return (
        <Modal close={closeModal}>
            <h2 className="text-2xl sm:text-3xl font-bold text-center text-white mb-2">Create New Workspace</h2>
            <p className="text-center text-slate-400 mb-6 text-sm sm:text-base">Organize your projects and tasks.</p>
            <form className="space-y-4" onSubmit={handleSubmit}>
                <FormInput label="Workspace Name" type="text" value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g. Q4 Marketing Campaign" required />
                <FormTextarea label="Description (optional)" value={description} onChange={(e) => setDescription(e.target.value)} placeholder="A brief description of this workspace." />
                <PrimaryButton type="submit" isLoading={isLoading}>Create Workspace</PrimaryButton>
...
const CreateTeamModal = ({ closeModal, handleCreateTeam, isLoading }) => {
    ...
    return (
        <Modal close={closeModal}>
            <h2 className="text-2xl sm:text-3xl font-bold text-center text-white mb-2">Create New Team</h2>
            <p className="text-center text-slate-400 mb-6 text-sm sm:text-base">Bring your team members together.</p>
            <form className="space-y-4" onSubmit={handleSubmit}>
                <FormInput label="Team Name" type="text" value={name} onChange={e => setName(e.target.value)} placeholder="e.g. Design Crew" required />
                <FormTextarea label="Description (optional)" value={description} onChange={e => setDescription(e.target.value)} placeholder="What this team is about." />
                <PrimaryButton type="submit" isLoading={isLoading}>Create Team</PrimaryButton>
...
const NotificationsPanel = ({ notifications, onMarkAsRead, isOpen }) => {
    return (
        <Transition
            ...
            leaveTo="transform opacity-0 scale-95"
        >
            <div className="absolute right-4 sm:right-0 mt-2 w-[calc(100vw-2rem)] max-w-sm sm:w-80 bg-slate-700/80 backdrop-blur-md border border-slate-600 rounded-lg shadow-lg py-1 z-30">
                <div className="px-4 py-2 font-bold text-white">Notifications</div>
                <div className="max-h-96 overflow-y-auto">
                    {notifications.length > 0 ? notifications.map(notif => (
                        <div
...
const ProfileDropdown = ({ user, onEditProfile, onChangePassword, onLogout, isOpen }) => (
    <Transition
        ...
        leaveTo="transform opacity-0 scale-95"
    >
        <div className="absolute right-4 sm:right-0 mt-2 w-56 bg-slate-700/80 backdrop-blur-md border border-slate-600 rounded-lg shadow-lg py-2 z-20">
            <div className="px-4 py-2 border-b border-slate-600">
                <p className="text-sm font-semibold text-white">{user.name}</p>
                <p className="text-xs text-slate-400 truncate">{user.email}</p>
            </div>
...
const Header = ({ user, unreadCount, onProfileToggle, onNotificationToggle }) => (
    <header className="bg-slate-800/50 backdrop-blur-md border-b border-slate-700 flex items-center justify-between px-4 sm:px-8 py-3 sticky top-0 z-10 gap-4">
        <div className="md:hidden">
            <h1 className="text-xl font-bold text-white">SyncSpace</h1>
        </div>
        <div className="relative flex-1 min-w-0 hidden md:block">
            <Search size={20} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-slate-400" />
            <input type="text" placeholder="Search..." className="w-full bg-slate-700 border border-slate-600 rounded-lg pl-10 pr-4 py-2 focus:outline-none focus:ring-2 focus:ring-teal-500 transition" />
        </div>
        <div className="flex items-center gap-4 sm:gap-6">
            <button onClick={onNotificationToggle} className="relative text-slate-400 hover:text-white transition-colors">
                <Bell size={22} />
                {unreadCount > 0 && <span className="absolute -top-1 -right-1.5 flex h-4 w-4 sm:h-5 sm:w-5 items-center justify-center bg-red-500 text-xs rounded-full">{unreadCount}</span>}
            </button>
            <button onClick={onProfileToggle} className="flex items-center gap-2">
                <div className="w-8 h-8 sm:w-9 sm:h-9 rounded-full bg-teal-500 flex items-center justify-center font-bold text-sm sm:text-base">{user?.name?.charAt(0)}</div>
            ...
            {navItems.map(item => (
                <Link 
                    ...
                    className={`flex flex-col items-center justify-center gap-1 w-full py-1 rounded-md transition-colors text-slate-400 hover:text-teal-400
                        ${item.label === 'Dashboard'? "bg-slate-700": ""}
                        `}
                >
                    <item.icon size={20} />
        ...
        <div className="font-sans min-h-screen bg-slate-900 text-white flex">
            ...
            <div className="flex-1 flex flex-col min-w-0">
                ...
                <main className="flex-1 p-4 sm:p-6 lg:p-8 overflow-y-auto pb-24 md:pb-8">
                    {alert && <div className={`fixed top-20 left-4 right-4 sm:left-auto sm:right-8 z-[60] bg-slate-700 border text-white text-sm py-2 px-4 rounded-lg shadow-lg animate-fade-in-down ${alert.type === 'error' ? 'border-red-500/50' : 'border-green-500/50'}`}>{alert.message}</div>}
                    ...
                    <p className="text-slate-400 mb-8 text-sm sm:text-base">Here's a snapshot of your collaborative space.</p>

                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                        <div className="lg:col-span-2 space-y-8">
                            <div>
                                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 gap-3">
                                    <h2 className="text-xl sm:text-2xl font-bold flex items-center gap-2"><Briefcase className="text-teal-400"/> Workspaces</h2>
                                    {user.role === 'Admin' && <button onClick={() => openModal("workspace")} className="flex items-center gap-2 px-3 py-2 text-sm sm:text-base sm:px-4 bg-teal-600 rounded-lg font-semibold hover:bg-teal-500 transition-transform hover:scale-105"><Plus size={18} /> New Workspace</button>}
                                </div>
                                {loading.workspaces ? <Loader2 className="animate-spin text-teal-500" /> :
                                    workspaces.length > 0 ? (
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                            {workspaces.map(ws => (
                                                <div key={ws._id} className="bg-slate-800 p-5 rounded-xl shadow-lg hover:shadow-teal-500/20 hover:-translate-y-1 transition-all cursor-pointer border border-slate-700 hover:border-teal-500/50" onClick={() => navigate(`/Workspace/${ws._id}`)}>
                                                    <h3 className="text-base sm:text-lg font-bold text-white mb-2">{ws.name}</h3>
                                                    <p className="text-slate-400 text-sm h-10 overflow-hidden">{ws.description || "No description provided."}</p>
                                                </div>
                                            ))}
                                        </div>
                                  ) : user.role === 'Admin'? (<div className="text-center py-10 bg-slate-800 rounded-lg border border-slate-700 text-sm sm:text-base p-4">No workspaces yet. Create one to get started!</div>):
                                  (<div className="text-center py-10 bg-slate-800 rounded-lg border border-slate-700 text-sm sm:text-base p-4">You are not added to any workspace. Contact the relevant Admin for further information.</div>)
                                }
                            </div>

                            <div>
                                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 gap-3">
                                    <h2 className="text-xl sm:text-2xl font-bold flex items-center gap-2"><Users className="text-teal-400"/> Teams</h2>
                                    {user.role === 'Admin' && <button onClick={() => openModal("team")} className="flex items-center gap-2 px-3 py-2 text-sm sm:text-base sm:px-4 bg-slate-700 rounded-lg font-semibold hover:bg-slate-600 transition-transform hover:scale-105"><Plus size={18} /> New Team</button>}
                                </div>
                                {loading.teams ? <Loader2 className="animate-spin text-teal-500" /> :
                                    teams.length > 0 ? (
                                        <div className="bg-slate-800 p-3 rounded-xl border border-slate-700">
                                            {teams.map(team => (
                                                <div key={team._id} className="flex justify-between items-center p-3 border-b border-slate-700 last:border-none">
                                                    ...
                                                    <span className="text-slate-400 text-xs sm:text-sm">{team.members.length} members</span>
                                                </div>
                                            ))}
                                        </div>
                                    ) : <div className="text-center py-10 bg-slate-800 rounded-lg border border-slate-700 text-sm sm:text-base p-4">You are not part of any team yet.</div>
                                }
                            </div>
                        </div>
                        ...
                        <div className="space-y-8">
                            <div className="bg-slate-800 p-4 sm:p-6 rounded-xl shadow-lg border border-slate-700">
                                <h2 className="text-lg sm:text-xl font-bold mb-4 flex items-center gap-2"><FolderKanban className="text-teal-400"/> Task Summary</h2>
                                {loading.tasks ? <Loader2 className="animate-spin text-teal-500"/> :
                                <div className="space-y-3">
                                    {Object.entries(tasksSummary).map(([status, count]) => (
                                        <div key={status} className="flex justify-between items-center text-slate-300 text-sm sm:text-base">
                                            <span>{status}</span>
                                            <span className="font-bold bg-slate-700 px-2 py-0.5 rounded text-white">{count}</span>
                                        </div>
                                    ))}
                            ...
                            <div className="bg-slate-800 p-4 sm:p-6 rounded-xl shadow-lg border border-slate-700">
                                <h2 className="text-lg sm:text-xl font-bold mb-4 flex items-center gap-2"><MessageCircle className="text-teal-400"/> Recent Activity</h2>
                                <ul className="space-y-3">
                                    <li className="flex items-start sm:items-center gap-3 text-sm text-slate-400"><Clock size={16} className="text-teal-500 flex-shrink-0 mt-1 sm:mt-0" /> Task "Landing Page" marked Completed</li>
                                    <li className="flex items-start sm:items-center gap-3 text-sm text-slate-400"><Plus size={16} className="text-teal-500 flex-shrink-0 mt-1 sm:mt-0" /> Workspace "Marketing" created</li>
                                </ul>
                            </div>
                        </div>
                    </div>
