...
//           <Link to={`/Team/${user._id}`} className="flex items-center gap-3 p-3 rounded-lg hover:bg-slate-700 text-slate-300"><LayoutGrid size={20}/> Teams</Link>
//           {user.role === 'Admin' && (
...
const DashboardPage = () => {
    ...
    const createApiHandler = (endpoint, successMessage, errorMessage, setData) => async (data) => {
        ...
        try {
            const res = await axios.post(`${API_BASE_URL}/${endpoint}`, data, { headers: { Authorization: `Bearer ${token}` } });
    ...
    const updateApiHandler = (endpoint, successMessage, errorMessage, setData) => async (data) => {
        setModal(prev => ({ ...prev, isLoading: true }));
        try {
            const res = await axios.put(`${API_BASE_URL}/${endpoint}`, data, { headers: { Authorization: `Bearer ${token}` } });
