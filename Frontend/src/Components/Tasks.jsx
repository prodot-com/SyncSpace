...
import {
  Loader2, CheckSquare, LayoutGrid, X, Send,
} from "lucide-react";
...
const ModalShell = ({ children, close }) => (
  <div className="fixed inset-0 bg-black bg-opacity-70 z-[60] flex justify-center items-center p-4">
    <div className="bg-slate-800 rounded-2xl shadow-2xl p-6 w-full max-w-2xl relative animate-fade-in-up">
      <button
        onClick={close}
...
const TaskCard = ({ task, onOpenDetails }) => {
  ...
  return (
    <div
      ...
      className="bg-slate-700 p-3 rounded-lg mb-3 cursor-pointer"
    >
      <div className="flex justify-between items-start">
        <div className="flex-1">
          <p className="font-semibold text-white">{task.title}</p>
          <p className="text-xs text-slate-400 mt-1">in <span className="font-semibold">{task.workspace.name}</span></p>
        </div>
      </div>
    </div>
  );
      ...
      <div className="flex justify-between items-start mb-4">
        <div>
          ...
          <p className="text-sm text-slate-400">
            in workspace: <span className="font-semibold text-teal-400">{task.workspace.name}</span>
          </p>
        </div>
        <select
          value={task.status}
          onChange={(e) => onUpdateStatus(task, e.target.value)}
          className="px-3 py-2 rounded bg-slate-700 border border-slate-600 appearance-none"
        >
          <option>To Do</option>
...
// --- New Task Modal ---
const NewTaskModal = ({ close, workspaces, onCreate }) => {
  ...
  return (
    <ModalShell close={close}>
      ...
      <form className="space-y-3" onSubmit={handleSubmit}>
        ...
        <button type="submit" className="bg-teal-600 p-2 rounded w-full hover:bg-teal-500 transition">Create Task</button>
      </form>
    </ModalShell>
  );
};
...
const TasksPage = () => {
  ...
  const fetchAllTasks = useCallback(async () => {
    try {
      ...
      const taskPromises = wsRes.data.map(ws =>
  ...
  task =>
    ...
    //   console.log("MyTasks:", myTasks)
      ...
      myTasks.forEach(task => {
        ...
        if (ws) task.workspace = { _id: ws._id, name: ws.name };
      });
...
const createTask = async ({ title, description, workspace }) => {
  try {
  ...
  const handleDragEnd = (event) => {
    ...
    if (!activeContainer || !overContainer || active.id === over.id || activeContainer === overContainer) return;
  ...
  return (
    <div className="font-sans min-h-screen bg-slate-900 text-white flex">
      ...
      {modal.type === "newTask" && <NewTaskModal close={closeModal} workspaces={workspaces} onCreate={createTask} />}
      ...
      <main className="flex-1 flex flex-col">
        <header className="bg-slate-800/50 shadow-md px-8 py-4 border-b border-slate-700 flex justify-between items-center">
          <div>
            <h1 className="text-2xl font-bold">My Tasks</h1>
            <p className="text-slate-400">All tasks assigned to you across all workspaces.</p>
          </div>
          <button onClick={() => setModal({ type: "newTask" })} className="bg-teal-600 px-4 py-2 rounded-lg hover:bg-teal-500 transition">+ New Task</button>
        </header>
