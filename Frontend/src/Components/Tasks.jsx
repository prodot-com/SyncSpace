...
const TaskCard = ({ task, onOpenDetails }) => {
    ...
    return (
        <div ref={setNodeRef} style={style} className="bg-slate-700 p-3 rounded-lg mb-3 cursor-pointer border border-slate-600 hover:border-teal-500/50 hover:bg-slate-600/50 transition-all group" onClick={() => onOpenDetails(task)}>
            <div className="flex justify-between items-start">
                <p className="font-semibold text-white flex-1 pr-2 text-sm">{task.title}</p>
                <div {...attributes} {...listeners} className="cursor-grab active:cursor-grabbing p-1 text-slate-500 group-hover:text-slate-300">
                    <GripVertical size={16}/>
                </div>
            </div>
    ...
    return (
        <Modal close={close}>
            <div className="flex flex-col gap-4 sm:flex-row sm:justify-between sm:items-start mb-4">
                ...
                <FormSelect value={task.status} onChange={(e) => onUpdateStatus(task, e.target.value)} className="w-full sm:w-auto !py-1 text-sm">
                    <option>To Do</option>
                    <option>In Progress</option>
                    <option>Completed</option>
                </FormSelect>
            </div>
            <p className="text-slate-300 mb-6 text-sm sm:text-base">{task.description || "No description provided."}</p>
            <h3 className="font-bold text-lg mb-3 flex items-center gap-2"><MessageSquare size={18} /> Comments</h3>
            <div className="space-y-4 max-h-64 overflow-y-auto bg-slate-900/50 p-3 rounded-lg border border-slate-700">
                {loadingComments ? <div className="flex justify-center p-4"><Loader2 className="animate-spin" /></div> :
                    comments.length > 0 ? comments.map(comment => (
...
const NewTaskModal = ({ close, workspaces, onCreate, isLoading }) => {
    ...
    return (
        <Modal close={close} size="md">
            <h2 className="text-xl sm:text-2xl font-bold mb-4 text-center">Create New Task</h2>
            <form className="space-y-4" onSubmit={handleSubmit}>
                <FormInput label="Task Title" value={title} onChange={(e) => setTitle(e.target.value)} placeholder="What needs to be done?" required/>
                <FormTextarea label="Description" value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Add more details..."/>
                <FormSelect label="Workspace" value={workspaceId} onChange={(e) => setWorkspaceId(e.target.value)} required>
    ...
    const handleDragEnd = ({ active, over }) => {
        ...
        if (activeContainer === overContainer) {
            setTasks(prevTasks => {
                ...
                if (oldIndex !== -1 && newIndex !== -1) {
                    const reorderedInColumn = arrayMove(activeColumnItems, oldIndex, newIndex);
                    const newTasks = [...prevTasks];
                    reorderedInColumn.forEach(item => {
                        const taskIndex = newTasks.findIndex(t => t._id === item._id);
                        if (taskIndex !== -1) newTasks[taskIndex] = item;
                    });
                    return newTasks;
                }
                return prevTasks;
            ...
            const task = tasks.find(t => t._id === active.id);
            if (task) handleUpdateStatus(task, overContainer);
        }
    };
    ...
    return (
        <div className="font-sans min-h-screen bg-slate-900 text-white flex">
            ...
            <main className="flex-1 flex flex-col h-screen overflow-hidden">
                <header className="bg-slate-800/50 backdrop-blur-md shadow-md px-4 sm:px-8 py-4 border-b border-slate-700 flex flex-col items-start gap-4 sm:flex-row sm:justify-between sm:items-center flex-shrink-0">
                    <div>
                        <h1 className="text-xl sm:text-2xl font-bold">My Tasks</h1>
                        <p className="text-slate-400 text-sm">Tasks assigned to you across all workspaces.</p>
                    </div>
                    <button onClick={() => setModal({ type: "newTask" })} className="flex items-center gap-2 bg-teal-600 px-3 py-2 text-sm sm:px-4 sm:text-base rounded-lg font-semibold hover:bg-teal-500 transition-transform hover:scale-105">
                        <Plus size={18} /> 
                        <span className="hidden sm:inline">New Task</span>
                    </button>
                </header>

                <div className="flex-1 p-4 md:p-8 overflow-x-auto overflow-y-hidden pb-24 md:pb-8">
                    <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
                        <div className="flex gap-4 sm:gap-6 h-full">
                            {columns && Object.entries(columns).map(([colId, col]) => (
                                <div key={colId} className="w-[80vw] sm:w-80 lg:w-96 flex-shrink-0 bg-slate-800/70 rounded-xl flex flex-col h-full border border-slate-700">
                                    <h3 className="font-semibold text-base sm:text-lg px-4 pt-4 pb-2 flex items-center gap-2 border-b border-slate-700">
                                        {col.name}
                                        <span className="text-sm bg-slate-700 text-slate-300 rounded-full px-2 py-0.5">{col.items.length}</span>
                                    </h3>
                                    <SortableContext items={col.items.map(i => i._id)} strategy={verticalListSortingStrategy}>
                                        <div className="flex-1 overflow-y-auto p-2 sm:p-4">
                                            {col.items.length > 0 ?
                                                col.items.map(item => <TaskCard key={item._id} task={item} onOpenDetails={openTaskDetails} />)
                                                : <div className="text-center text-slate-500 pt-10 text-sm">No tasks here!</div>
                                            }
                                        </div>
                                    </SortableContext>
                                </div>
