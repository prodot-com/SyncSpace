...
const EditTaskModal = ({ close, task, members, onUpdate, onDelete }) => { const [title, setTitle] = useState(task.title || ""); const [description, setDescription] = useState(task.description || ""); const [status, setStatus] = useState(task.status || "To Do"); const [assignedTo, setAssignedTo] = useState(task.assignedTo ? task.assignedTo._id : ""); const submit = (e) => { e.preventDefault(); if (!title.trim()) return; onUpdate(task._id, { title, description, status, assignedTo: assignedTo || null }); }; return (<ModalShell close={close}><h2 className="text-2xl font-bold mb-3">Edit Task</h2><form onSubmit={submit} className="space-y-3"><input value={title} onChange={e => setTitle(e.target.value)} placeholder="Title" className="w-full px-3 py-2 rounded bg-slate-700" required /><textarea value={description} onChange={e => setDescription(e.target.value)} placeholder="Description" rows={4} className="w-full px-3 py-2 rounded bg-slate-700" /><div className="flex gap-2"><select value={status} onChange={e => setStatus(e.target.value)} className="px-3 py-2 rounded bg-slate-700"><option>To Do</option><option>In Progress</option><option>Completed</option></select><select value={assignedTo} onChange={e => setAssignedTo(e.target.value)} className="px-3 py-2 rounded bg-slate-700"><option value="">Unassigned</option>{members.map(m => <option key={m._id} value={m._id}>{m.name}</option>)}</select></div><div className="flex gap-2 pt-2"><button type="submit" className="bg-teal-600 px-4 py-2 rounded font-semibold hover:bg-teal-500 transition">Save Changes</button><button type="button" onClick={() => onDelete(task._id)} className="bg-red-600 px-4 py-2 rounded font-semibold hover:bg-red-500 transition text-white">Delete Task</button><button type="button" onClick={close} className="bg-slate-700 px-4 py-2 rounded font-semibold hover:bg-slate-600 transition">Cancel</button></div></form></ModalShell>);};
...
const WorkspacePage = () => {
  ...
  // --- API Actions ---
  const createTask = async (data) => { try { const res = await axios.post(`${API_BASE_URL}/tasks`, data, { headers: { Authorization: `Bearer ${token}` } }); if (socket) socket.emit("taskUpdated", res.data); closeModal(); } catch (err) { alert("Failed to create task"); }};
  ...
  const uploadDocument = async (file) => { const formData = new FormData(); formData.append("file", file); try { const res = await axios.post(`${API_BASE_URL}/documents/${workspaceId}`, formData, { headers: { Authorization: `Bearer ${token}`, "Content-Type": "multipart/form-data" }}); if(socket) socket.emit("docUpdated", {doc: res.data}); } catch (err) { alert("File upload failed"); }};
  const createTextDocument = async (title) => { try { const res = await axios.post(`${API_BASE_URL}/documents/text`, { title, workspaceId }, { headers: { Authorization: `Bearer ${token}` }}); if(socket) socket.emit("docUpdated", {doc: res.data}); setActiveDocument(res.data._id); closeModal(); } catch (err) { alert("Failed to create document."); }};
  const deleteDocument = (docId) => { openModal("confirm", { title: "Delete Document?", message: "Are you sure you want to permanently delete this file?", onConfirm: async () => { try { await axios.delete(`${API_BASE_URL}/documents/${docId}`, { headers: { Authorization: `Bearer ${token}` } }); if(socket) socket.emit("docUpdated", {docId: docId, deleted: true}); closeModal(); } catch (err) { alert("Failed to delete document"); } } }); };
