...
const CreateTeamModal = ({ closeModal, handleCreate }) => {
    ...
    return (
        <ModalShell close={closeModal}>
            <h2 className="text-xl sm:text-2xl font-bold mb-4">Create New Team</h2>
            <form className="space-y-3" onSubmit={handleSubmit}>
                <input
                    type="text"
                    value={name}
...
const EditTeamModal = ({ closeModal, team, handleUpdate }) => {
    ...
    return (
        <ModalShell close={closeModal}>
            <h2 className="text-xl sm:text-2xl font-bold mb-4">Edit Team</h2>
            <form className="space-y-3" onSubmit={handleSubmit}>
                <input
                    type="text"
                    value={name}
...
const ManageMembersModal = ({ closeModal, team, allUsers, handleAddMember, handleRemoveMember }) => {
    ...
    return (
        <ModalShell close={closeModal}>
            <h2 className="text-xl sm:text-2xl font-bold mb-4">Manage Members for {team.name}</h2>
            <form onSubmit={handleAdd} className="mb-4">
...
const ViewMembersModal = ({ closeModal, team }) => (
    <ModalShell close={closeModal}>
        <h2 className="text-xl sm:text-2xl font-bold mb-4">Members of {team.name}</h2>
        <div className="space-y-2 max-h-64 overflow-y-auto pr-2">
            {team.members.map(member => (
                <div key={member._id} className="flex items-center justify-between p-3 bg-slate-900/50 rounded-lg">
                    <div>
...
const EditProfileModal = ({ closeModal, user, handleUpdateProfile }) => {
    ...
    return (<ModalShell close={closeModal}><h2 className="text-xl sm:text-2xl font-bold mb-4">Edit Profile</h2><form className="space-y-3" onSubmit={handleSubmit}>{["name","email","skills","portfolio","rate"].map((field) => ( <input key={field} type="text" name={field} value={formData[field]} onChange={handleChange} placeholder={field.charAt(0).toUpperCase() + field.slice(1)} className="w-full px-4 py-2 bg-slate-700 rounded-lg"/>))}<button type="submit" className="w-full py-2 bg-teal-600 rounded-lg hover:bg-teal-500">Update</button></form></ModalShell>);
};
...
const NotificationsPanel = ({ notifications, onMarkAsRead, navigate }) => {
    ...
    return (<div className="absolute right-4 sm:right-0 mt-2 w-[calc(100vw-2rem)] max-w-sm sm:w-80 bg-slate-700 rounded-lg shadow-lg py-2 z-30"><div className="px-4 py-2 font-bold text-white border-b border-slate-600">Notifications ({unreadCount})</div><div className="max-h-96 overflow-y-auto">{notifications.length > 0 ? notifications.map(notif => ( <div key={notif._id} className={`px-4 py-3 border-b border-slate-600 last:border-b-0 cursor-pointer hover:bg-slate-600 ${!notif.read ? 'bg-teal-500/10' : ''}`} onClick={() => { onMarkAsRead(notif._id);}}><p className="text-sm text-slate-200">{notif.message}</p><p className="text-xs text-slate-400 mt-1">{new Date(notif.createdAt).toLocaleString()}</p></div>)) : <p className="text-sm text-slate-400 text-center py-4">No new notifications.</p>}</div></div>);
};
        ...
        <div className="font-sans min-h-screen bg-slate-900 text-white flex">
            ...
            <aside className="w-64 bg-slate-800 p-6 flex-col border-r border-slate-700 hidden md:flex">
                ...
                <nav className="flex flex-col space-y-2">
                    <Link to={`/Dashboard/${currentUser._id}`} className="flex items-center gap-3 p-3 rounded-lg hover:bg-slate-700 text-slate-300"><LayoutGrid size={20}/> Dashboard</Link>
                    <Link to={`/Tasks/${currentUser._id}`} className="flex items-center gap-3 p-3 rounded-lg hover:bg-slate-700 text-slate-300"><CheckSquare size={20}/> My Tasks</Link>
                    <Link to={`/Team/${currentUser._id}`} className="flex items-center gap-3 p-3 rounded-lg bg-teal-500/20 font-semibold text-white"><Users size={18} /> Teams</Link>
                    {currentUser.role === 'Admin' && (
                        <Link to={`/Admin/${currentUser._id}`}className="flex items-center gap-3 p-3 rounded-lg hover:bg-slate-700 text-slate-300"><Shield size={20}/> Admin Panel</Link>
                    )}
                </nav>
            ...
            <main className="flex-1 flex flex-col min-w-0">
                 <header className="bg-slate-800/50 shadow-md flex items-center justify-between px-4 sm:px-8 py-3">
                    ...
                    <div className="flex items-center gap-4">
                        <div className="relative">
                           <button onClick={() => { setNotificationsOpen(p => !p); setProfileOpen(false); }} className="relative text-slate-400 hover:text-white">
                                <Bell size={22} />
                                {unreadCount > 0 && <span className="absolute -top-1 -right-1.5 flex h-4 w-4 sm:h-5 sm:w-5 items-center justify-center rounded-full bg-red-500 text-xs">{unreadCount}</span>}
                            </button>
                        ...
                        <div className="relative">
                            <button onClick={() => { setProfileOpen(p => !p); setNotificationsOpen(false); }} className="flex items-center gap-2">
                                <div className="w-8 h-8 sm:w-9 sm:h-9 rounded-full bg-teal-500 flex items-center justify-center font-bold text-sm sm:text-base">{currentUser?.name?.charAt(0)}</div>
                            </button>
                            {profileOpen && (
                                <div className="absolute right-0 mt-2 w-48 bg-slate-700 rounded-lg shadow-lg py-2 z-20">
                                    <button onClick={() => openModal("editProfile")} className="w-full flex items-center gap-3 px-4 py-2 text-sm text-slate-300 hover:bg-slate-600"><Edit3 size={16}/> Edit Profile</button>
                ...
                <div className="p-4 sm:p-8 overflow-y-auto pb-24 md:pb-8">
                    <div className="flex items-center justify-end mb-6">
                        {currentUser?.role === 'Admin' && (
                            <button
                                onClick={() => openModal('createTeam')}
                                className="bg-teal-600 px-3 py-2 sm:px-4 rounded-lg flex items-center gap-2 font-semibold hover:bg-teal-500 transition text-sm"
                            >
                                <Plus size={16}/> New Team
                            </button>
                        )}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                        {teams.map(team => (
                            <div key={team._id} className="bg-slate-800 p-6 rounded-xl shadow-lg flex flex-col">
                                ...
                                <div className="border-t border-slate-700 mt-4 pt-4 flex flex-col sm:flex-row gap-2">
                                    {currentUser?.role === 'Admin' ? (
                                        <>
                                            <button
                                                onClick={() => openModal('manageMembers', {team})}
                                                className="flex-1 text-center text-sm py-2 bg-slate-700 hover:bg-slate-600 rounded-lg transition"
                                            >
                                                Manage Members
                                            </button>
                                            <div className="flex gap-2">
                                                <button
                                                    onClick={() => openModal('editTeam', {team})}
                                                    className="flex-1 sm:flex-none p-2 bg-slate-700 hover:bg-slate-600 rounded-lg transition"
                                                >
                                                    <Edit3 size={16}/>
                                                </button>
                                                <button
                                                    onClick={() => handleDeleteTeam(team._id)}
                                                    className="flex-1 sm:flex-none p-2 bg-red-500/20 text-red-400 hover:bg-red-500/30 rounded-lg transition"
                                                >
                                                    <Trash2 size={16}/>
                                                </button>
                                            </div>
